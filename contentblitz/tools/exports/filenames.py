...
def make_pdf_filename(seed_text: str) -> str:
    """Create deterministic pdf filename from a safe hash."""
    digest = sha256(_safe_text(seed_text).encode("utf-8")).hexdigest()[:12]
...
def resolve_pdf_export_path(
    seed_text: str,
    *,
    export_dir: str | Path | None = None,
) -> Path:
    """
    Resolve a pdf file path inside the configured export directory.
