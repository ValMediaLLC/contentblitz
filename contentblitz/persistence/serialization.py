...
def _sanitize_content_drafts(content_drafts: Any) -> Dict[str, Any]:
    ...
    return {
        "blog": {
            ...
            "version": (
                int(blog.get("version", 0))
                if isinstance(blog.get("version"), int)
                else 0
            ),
        },
        "linkedin": {
...
def _sanitize_export_metadata(export_metadata: Any) -> Dict[str, Any]:
    ...
    formats = [
        ...
        for token in [
            ...
            for item in _safe_list(meta.get("formats_requested", []))
        ]
        if token
    ]
...
def _sanitize_image_outputs(image_outputs: Any) -> List[Dict[str, Any]]:
    ...
    for item in safe:
        ...
        if isinstance(raw.get("url"), str) and raw["url"].strip().lower().startswith(
            "data:image/"
        ):
            continue
...
def _sanitize_ui_selected_options(options: Any) -> Dict[str, Any]:
    ...
    export_formats = [
        ...
        for token in [
            _safe_text(item).lower()
            for item in _safe_list(raw.get("export_formats", []))
        ]
        if token
    ]
    return {
...
def _sanitize_export_paths_for_restore(
    export_paths: Mapping[str, Any],
...
def serialize_workflow_run(
    *,
    ...
    safe_progress_events = _sanitize_progress_events(
        progress_events or _safe_list(state.get("ui_progress_events", []))
    )
    safe_status_messages = _sanitize_status_messages(
        status_messages or _safe_list(state.get("status_messages", []))
    )
    ui_options = _sanitize_ui_selected_options(
        ui_selected_options or state.get("ui_selected_options", {})
    )
    ui_node_statuses = _sanitize_node_statuses(state.get("ui_node_statuses", {}))
    ui_workflow_status = _sanitize_text(
        state.get("ui_workflow_status") or state.get("workflow_status")
    )
...
def deserialize_workflow_run(record: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    requested_outputs = _sanitize_requested_outputs(
    ...
    export_paths, missing_paths = _sanitize_export_paths_for_restore(
        export_metadata.get("export_paths", {})
    )
    ...
    restored_state = {
        ...
        "content_drafts": content_drafts,
        "partial_outputs": partial_outputs,
        "partial_output_mode": _sanitize_text(payload.get("partial_output_mode"))
        or "none",
        "image_prompts": [
            _sanitize_text(item)
            for item in _safe_list(payload.get("image_prompts", []))
            if _sanitize_text(item)
        ],
        "image_outputs": image_outputs,
        "sources": sources,
        ...
        "ui_progress_events": _sanitize_progress_events(
            payload.get("progress_events", [])
        ),
        "status_messages": _sanitize_status_messages(
            payload.get("status_messages", [])
        ),
        "ui_selected_options": _sanitize_ui_selected_options(
            payload.get("ui_selected_options", {})
        ),
        "ui_node_statuses": _sanitize_node_statuses(
            payload.get("ui_node_statuses", {})
        ),
        "prompt_injection_detected": bool(
            payload.get("prompt_injection_detected", False)
        ),
        "prompt_injection_signals": _sanitize_injection_signals(
...
def to_run_summary(record: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    preview = (
        user_query if len(user_query) <= 120 else user_query[:117].rstrip() + "..."
    )
