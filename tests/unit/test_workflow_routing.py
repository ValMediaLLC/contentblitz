...
def test_content_strategist_route_order_is_deterministic_for_full_fanout() -> None:
    state = create_initial_state(
        requested_outputs=["image", "blog", "linkedin", "blog"]
    )
    route = route_from_content_strategist(state)
    assert route == [BLOG_WRITER_NODE, LINKEDIN_WRITER_NODE, IMAGE_AGENT_NODE]


def test_content_strategist_defaults_to_blog_and_linkedin_when_outputs_missing() -> (
    None
):
...
def test_content_strategist_handles_invalid_requested_outputs_shape_deterministically() -> (
    None
):
