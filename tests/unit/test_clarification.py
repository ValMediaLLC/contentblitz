...
def test_extract_message_appends_question_mark_for_plain_text() -> None:
    message = clarification_module._extract_message(
        {"output": "Could you provide more detail"}
    )
...
def test_resolve_clarification_message_preserves_existing_value(monkeypatch) -> None:
    ...
    def _fail_if_called(*_args, **_kwargs):
        raise AssertionError(
            "generate_text should not be called when message already exists."
        )
...
def test_clarification_node_handles_empty_prompt_safely_without_draft_mutation() -> (
    None
):
    state = _base_state(
        user_query="   ", clarification_message="Can you clarify your request?"
    )
